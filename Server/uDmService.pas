...
USES
  ...
  UDWJSONObject, Dialogs, ServerUtils, FireDAC.Dapt, UDWConstsData,
  FireDAC.Phys.FBDef,
  ...
  RestDWServerFormU,
  URESTDWPoolerDB, URestDWDriverFD, FireDAC.Phys.MSSQLDef,
  FireDAC.Phys.ODBCBase,
  FireDAC.Phys.MSSQL, uDWConsts, uRESTDWServerEvents, uSystemEvents,
  FireDAC.Stan.Param,
  FireDAC.DatS, FireDAC.Dapt.Intf, FireDAC.Comp.DataSet, uDWAbout,
...
procedure TServerMethodDM.employeeReplyEvent(var Params: TDWParams;
...
begin
  ...
  try
    ...
    FDQuery3.SQL.Clear;
    FDQuery3.SQL.Text :=
      'select * from produto where codcategoria = :cat order by 2';
    FDQuery3.ParamByName('cat').AsInteger := Params.ItemsString['pCategoria']
      .AsInteger;
    FDQuery3.Open;
...
begin
  ...
  if Pos('like', Params.ItemsString['pSelect'].AsString) <> 0 then
    vSelect := 'select * from produto where ' + Params.ItemsString['pSelect']
      .AsString + '%' + '''' +
      ' order by 2'
  else
    vSelect := 'select * from produto where ' + Params.ItemsString['pSelect']
...
begin
  ...
  FDQuery3.SQL.Clear;
  FDQuery3.SQL.Text :=
    'select login from usuario where login = :login and senha = :senha';

  FDQuery3.ParamByName('login').AsString := Params.ItemsString
    ['pUsuario'].AsString;
  FDQuery3.ParamByName('senha').AsString := Params.ItemsString
    ['pSenha'].AsString;

  FDQuery3.Open;

  if FDQuery3.IsEmpty then
    Result := 'erro'
  else
    Result := FDQuery3.Params[0].AsString;
...
begin
  ...
  ConnectionDefs.DatabaseName := IncludeTrailingPathDelimiter
    (RestDWForm.EdPasta.Text) + ConnectionDefs.DatabaseName;
